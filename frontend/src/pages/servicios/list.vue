                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <router-link :to="`/servicios/add`">
                                <Button :label="$t('addNewServicios')" icon="pi pi-plus" type="button" class="p-button bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/servicios'}" :model="pageBreadCrumb" />
                            </template>
                                ...
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" :header="$t('id')" >
                                            <template #body="{data}">
                                                <router-link :to="`/servicios/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="titulo" :header="$t('titulo')" >
                                            <template #body="{data}">
                                                {{ data.titulo }}
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="imagen" :header="$t('imagen')" >
                                            <template #body="{data}">
                                                <image-viewer image-size="small" image-preview-size="" :src="data.imagen" width="50px" height="50px" class="img-fluid" :num-display="1">
                                                </image-viewer>
                                            </template>
                                        </Column>
                                        <Column  field="icono" :header="$t('icono')" >
                                            <template #body="{data}">
                                                <image-viewer image-size="small" image-preview-size="" :src="data.icono" width="50px" height="50px" class="img-fluid" :num-display="1">
                                                </image-viewer>
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'servicioslist',
		},
		apiPath: {
			type: String,
			default: 'servicios/index',
		},
		paginate: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/servicios/view/${data.id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/servicios/edit/${data.id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
