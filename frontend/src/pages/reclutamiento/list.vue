                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <router-link :to="`/reclutamiento/add`">
                                <Button :label="$t('addNewReclutamiento')" icon="pi pi-plus" type="button" class="p-button bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/reclutamiento'}" :model="pageBreadCrumb" />
                            </template>
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="posiciones" :header="$t('posiciones')" >
                                            <template #body="{data}">
                                                {{ data.posiciones }}
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="salario" :header="$t('salario')" >
                                            <template #body="{data}">
                                                {{ data.salario }}
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="telefono" :header="$t('telefono')" >
                                            <template #body="{data}">
                                                {{ data.telefono }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'reclutamientolist',
		},
		apiPath: {
			type: String,
			default: 'reclutamiento/index',
		},
		paginate: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/reclutamiento/view/${data.id}`,
			icon: "pi pi-eye"
		},
		{
			label: () => $t('edit'),
			to: `/reclutamiento/edit/${data.id}`,
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
	...
	onMounted(()=>{ 
		const pageTitle = $t('reclutamiento');
