...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'contactoview',
		},
		apiPath: {
			type: String,
			default: 'contacto/view',
		},
		editButton: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('edit'),
			command: (event) => { app.openPageDialog({ page:'contacto/edit', url: `/contacto/edit/${data.id}`, closeBtn: true }) },
			icon: "pi pi-pencil"
		},
		{
			label: () => $t('delete'),
	...
	onMounted(()=>{ 
		const pageTitle = $t('contactoDetails');
